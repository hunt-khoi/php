...
<body>
	...
	<div class="container">
		...
		<?php
			...
			if($action=='deleted'){
				echo "<div class='alert alert-success'>Nhân viên đã được xoá.</div>";
			}
			...
			$query = "SELECT MaNV, HoNV, TenNV, NgaySinh, GioiTinh, DiaChi, SoDT, Email FROM nhanvien ORDER BY MaNV ASC
			...
			$num = $stmt->rowCount();
			echo "<a href='ThemNhanVien.php' class='btn btn-primary m-b-1em'>Thêm nhân viên mới</a>";
			...
			if($num>0){
				echo "<table class='table table-hover table-responsive table-bordered'>";
					...
					while ($row = $stmt->fetch(PDO::FETCH_ASSOC)){
						...
						echo "<tr>";
							echo "<td>{$MaNV}</td>";
							echo "<td>$HoNV $TenNV</td>";
                            echo "<td>{$NgaySinh}</td>";
                            echo "<td align='center'><img src ='hinhanh/$GioiTinh.png' width='17' height='40'></td>";
                            echo "<td>{$DiaChi}</td>";
                            echo "<td>{$SoDT}</td>";
                            echo "<td>{$Email}</td>";
							echo "<td>";
								echo "<a href='SuaNhanVien.php?id={$MaNV}&page=$page' class='btn btn-info m-r-1em'>Sửa</a>";
								echo "<a href='#' onclick='delete_nv(\"$MaNV\");' class='btn btn-success'>Xoá</a>";
							echo "</td>";
						echo "</tr>";
					}
				echo "</table>";

				$query = "SELECT COUNT(*) AS total_rows FROM nhanvien";
				$stmt = $con->prepare($query);
				$stmt->execute();
				...
				$page_url="DanhSachNhanVien.php?";
				include_once "PhanTrang.php";
			}

			// nếu không tìm thấy bản ghi dữ liệu nào
			else{
				echo "<div class='alert alert-danger'>Không tìm thấy nhân viên nào.</div>";
			}
		?>
