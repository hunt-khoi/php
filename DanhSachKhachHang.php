...
<body>
	...
	<div class="container">
		...
		<?php
			...
			if($action=='deleted'){
				echo "<div class='alert alert-success'>Khách hàng đã được xoá.</div>";
			}
			...
			$query = "SELECT MaKH, HoTenKH, GioiTinh, DiaChi, SoDT, Email FROM khachhang ORDER BY MaKH ASC
			...
			if($num>0){
				echo "<table class='table table-hover table-responsive table-bordered'>";
					...
					while ($row = $stmt->fetch(PDO::FETCH_ASSOC)){
						...
						echo "<tr>";
                            ...
                            echo "<td>{$Email}</td>";
							echo "<td>";
								echo "<a href='SuaKhachHang.php?id={$MaKH}&page=$page' class='btn btn-info m-r-1em'>Sửa</a>";
								echo "<a href='#' onclick='delete_kh(\"$MaKH\");' class='btn btn-success'>Xoá</a>";
							echo "</td>";
						echo "</tr>";
					}
				echo "</table>";

				$query = "SELECT COUNT(*) AS total_rows FROM khachhang";
				$stmt = $con->prepare($query);
				$stmt->execute();
				...
				$page_url="DanhSachKhachHang.php?";
